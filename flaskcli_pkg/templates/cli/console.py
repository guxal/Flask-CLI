#!/usr/bin/python3
"""
Command interpreter for <[name-app]> project
"""
import cmd
from models import storage, CNC


class <[app]>Command(cmd.Cmd):
    """
    ...
    def __class_err(self, arg):
        ...
        error = 0
        if len(arg) == 0:
            print(<[app]>Command.ERR[0])
            error = 1
        else:
            if isinstance(arg, list):
                arg = arg[0]
            if arg not in CNC.keys():
                print(<[app]>Command.ERR[1])
                error = 1
        return error
    ...
    def __handle_update_err(self, arg):
        ...
        for k in storage_objs.keys():
            if arg[1] in k and arg[0] in k:
                key = k
        if len(arg) < 3:
            print(<[app]>Command.ERR[4])
        elif len(arg) < 4:
            print(<[app]>Command.ERR[5])
        else:
